...
module.exports = [
    ...
    async execute(interaction) {
      ...
      if (!schedule?.messageId)
        return interaction.editReply(
          "❌ Nenhuma marcação encontrada. Usa `/schedule-post` primeiro.",
        );
      const votes = await countScheduleVotes(
        interaction.client,
        schedule.messageId,
      );
      await interaction.editReply(
        `**Votos de hoje (bot excluído):**\n✅ Disponível: **${votes.yes}**\n❌ Indisponível: **${votes.no}**\n🤷 Maybe: **${votes.maybe}**\n🔫 Contratado: **${votes.contratado}**`,
      );
    },
  },
  ...
  // ── /pens-reset ──────────────────────────────
  {
    data: new SlashCommandBuilder()
      .setName("pens-reset")
      .setDescription(
        "Resetar as pens e o quadro de membros (início de semana)",
      )
    ...
    async execute(interaction) {
      ...
      store.clearMeta();
      await interaction.editReply(
        "✅ Pens resetadas, quadro de membros atualizado e meta limpa!",
      );
    },
  },
  ...
  // ── /weekly-report ───────────────────────────
  {
    ...
    async execute(interaction) {
      ...
      store.clearWeekly();
      await interaction.editReply(
        "✅ Relatório semanal publicado e totais resetados!",
      );
    },
  },
  // ── /schedule-test ───────────────────────────
